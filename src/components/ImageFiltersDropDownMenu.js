...
export default function ImageFiltersDropDownMenu({ items, selected, onClick }) {
    ...
    return (
        <div className="absolute right-1 top-10 py-4 px-2 w-40 text-[11pt] rounded-lg bg-foreground">
            <ul className="space-y-4">{listItems}</ul>
            <p className="mt-3 text-xs text-center text-gray/50">Select up to 2 at one time.</p>
        </div>
    );
}